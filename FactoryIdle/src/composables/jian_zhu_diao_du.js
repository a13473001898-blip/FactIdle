...
export function use建筑调度() {
    ...
    // ================= 增加机器分配 =================
    const 尝试增加分配 = (配方id, 建筑id, 期望增加数量, cid) => {
        if (!cid) {
        ...
        // 校验通过，下达命令让底层 Store执行扣减和增加
        // (注：这里暂时保留 cloud_item_dummy_cid 魔术字符串，以后重构库存时再剥离)
        const 扣除成功 = 库存.库存减少(建筑id, 实际可增加数量, 'cloud_item_dummy_cid');
        if (!扣除成功) return false;
        
        配方分配._增加数量(配方id, 建筑id, 实际可增加数量, cid);
        return true;
    };

    // ================= 减少机器分配 =================
    const 尝试减少分配 = (配方id, 建筑id, 期望减少数量, cid) => {
        if (!cid) {
            console.error('🚨 [致命防御] 尝试减少机器分配但未提供 cid!');
            return false;
        }

        const 当前已分配 = 配方分配.查询分配数量(配方id, 建筑id, cid);
        const 实际减少数量 = Math.floor(Math.min(期望减少数量, 当前已分配));
        
        if (实际减少数量 <= 0) return false;

        // 让底层账本记录变动
        配方分配._减少数量(配方id, 建筑id, 实际减少数量, cid);
    ...
    // ================= 切换机器状态 =================
    const 切换状态 = (配方id, 建筑id, cid) => {
        if (!cid) {
            console.error('🚨 [致命防御] 尝试切换机器状态但未提供 cid！');
            return false;
        }
        配方分配._切换状态(配方id, 建筑id, cid);
        return true;
    };
