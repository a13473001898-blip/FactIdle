            ...
            <n-flex justify="space-between" align="center">
              ...
              <n-button-group size="small">
                ...
                <div class="machine-count-display">
                  {{ 格式化数字(配方分配.查询分配数量(全局常量.默认产线ID, recipe.id, machineId, 殖民地系统.当前视角ID)) }}
                </div>
                <n-button @click="尝试增加分配(recipe.id, machineId, 1 * 倍率, 殖民地系统.当前视角ID)">+</n-button>
              </n-button-group>
            </n-flex>

            <div v-if="配方分配.查询分配数量(全局常量.默认产线ID, recipe.id, machineId, 殖民地系统.当前视角ID) > 0" class="machine-status-toggle">
              <n-button block size="small" secondary
                :type="配方分配.查询建筑状态(全局常量.默认产线ID, recipe.id, machineId, 殖民地系统.当前视角ID) === '运行' ? 'success' : 'error'"
                @click="切换状态(recipe.id, machineId, 殖民地系统.当前视角ID, 全局常量.默认产线ID)">
                {{ 配方分配.查询建筑状态(recipe.id, machineId, 殖民地系统.当前视角ID, 全局常量.默认产线ID) === '运行' ? '✅ 运行中' : '⏸ 已停工' }}
              </n-button>
            </div>
          </div>
