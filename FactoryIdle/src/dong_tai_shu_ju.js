...
export function 查询配方分配() {
    return 游戏数据.配方分配
}

export function 查询配方分配建筑(配方id, 建筑id) {
    ...
    for (const 配方ID in 游戏数据.配方分配) {
        ...
        for (const 建筑ID in 游戏数据.配方分配[配方ID]) {
            ...
            if (建筑数据 && 建筑数据.能源类型 === 能源类型) {
                //得到相对应的建筑分配情况
                const 分配状态 = 游戏数据.配方分配[配方ID][建筑ID]
