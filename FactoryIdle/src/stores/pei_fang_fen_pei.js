...
export const use配方分配 = defineStore('pei_fang_fen_pei', {
  ...
  actions: {
    ...
    // 获取特定生产线的数据
    查询生产线(lineId = 'default', colonyId) {
      return this.数据[读取cid(colonyId)]?.[lineId] || {}
    },

    查询建筑(lineId = 'default', 配方id, colonyId) {
      return this.数据[读取cid(colonyId)]?.[lineId]?.[配方id] || {}
    },

    查询分配数量(lineId = 'default', 配方id, 建筑id, colonyId) {
      return this.数据[读取cid(colonyId)]?.[lineId]?.[配方id]?.[建筑id]?.数量 || 0
    },

    查询建筑状态(lineId = 'default', 配方id, 建筑id, colonyId) {
    ...
    _增加数量(lineId = 'default', 配方id, 建筑id, 数量, colonyId) {
      const cid = colonyId
    ...
    _减少数量(lineId = 'default', 配方id, 建筑id, 数量, colonyId) {
      const cid = colonyId
    ...
    _切换状态(lineId = 'default', 配方id, 建筑id, colonyId) {
      const cid = colonyId
    ...
    初始化新殖民地(cid) {
      if (!cid) return
      if (!this.数据[cid]) this.数据[cid] = { 'default': {} }
    },
    ...
    导入数据(存档数据) {
      ...
      const 迁移后数据 = {}
      // 检查是否是老版本二维结构，如果是，自动全部塞入 'default' 中转正
