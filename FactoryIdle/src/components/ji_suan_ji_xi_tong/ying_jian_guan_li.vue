  ...
  <n-card size="small" title="🖥️ 物理机箱组件" style="flex: 1; border-radius: 8px; overflow-y: auto;">
    <div class="slot-group">
      ...
      <div v-else class="hardware-empty">
          <n-select v-model:value="待装主板" :options="可用主板选项" placeholder="选择库存主板..." size="small" style="flex: 1;" />
    ...
    <div class="slot-group" :style="{ opacity: 计算机.装备的主板 ? 1 : 0.4 }">
      ...
      <div v-else-if="计算机.槽位限制.CPU > 0" class="hardware-empty">
          <n-select :value="null" :options="可用CPU选项" placeholder="空槽位 - 点击插上CPU..." size="small" @update:value="(val) => 计算机.安装CPU(val)" />
      </div>
    </div>
    ...
    <div class="slot-group" :style="{ opacity: 计算机.装备的主板 ? 1 : 0.4 }">
      ...
      <div v-for="(id, index) in 计算机.装备的内存" :key="'mem-'+index" class="hardware-item">
          ...
          <n-tag size="small" type="info" :bordered="false">+{{格式化字节(获取物品数据(id)?.提供算力) }} 内存</n-tag>
      ...
      <div v-for="i in 剩余内存槽位" :key="'empty-mem-'+i" class="hardware-empty">
          <n-select :value="null" :options="可用内存选项" placeholder="空槽位 - 点击插上内存..." size="small" @update:value="(val) => 计算机.安装内存(val)" />
      </div>
    </div>
    ...
    <div class="slot-group" :style="{ opacity: 计算机.装备的主板 ? 1 : 0.4 }">
      ...
      <div v-for="i in 剩余硬盘槽位" :key="'empty-hdd-'+i" class="hardware-empty">
          <n-select :value="null" :options="可用硬盘选项" placeholder="空槽位 - 点击插上硬盘..." size="small" @update:value="(val) => 计算机.安装硬盘(val)" />
      </div>
    </div>
  </n-card>
</template>
...
const 可用内存选项 = computed(() => {
    return 所有物品数组.filter(item => item.提供算力 > 0 && item.平台 === 计算机.当前平台 && 库存.查询库存(item.id) >= 1)
        .map(item => ({ label: `${item.名称} (+${格式化字节(item.提供算力)})`, value: item.id }));
});
...
.hardware-item {
  ...
  padding: 10px 12px; background-color: #f4fce3; border: 1px solid #18a058;
  border-radius: 6px; margin-bottom: 8px; transition: all 0.2s;
}
.hardware-empty {
  display: flex; justify-content: space-between; align-items: center;
  padding: 8px 12px; background-color: #fafafc; border: 1px dashed #d9d9d9;
  border-radius: 6px; margin-bottom: 8px; gap: 12px;
}
</style>