    ...
    <n-card size="small" title="🖥️ 物理机箱组件" style="flex: 1; border-radius: 8px; overflow-y: auto;">
        <div class="slot-group">
            ...
            <div v-if="计算机._当前机箱.装备的主板" class="hardware-item">
                ...
                <n-button size="tiny" type="error" ghost @click="卸载主板(殖民地系统.当前视角ID)">拔出</n-button>
            </div>
            <div v-else class="hardware-empty">
                <n-select v-model:value="待装主板" :options="可用主板选项" placeholder="选择库存主板..." size="small"
                    style="flex: 1;" />
                <n-button size="small" type="primary" @click="执行安装主板(殖民地系统.当前视角ID)" :disabled="!待装主板">安装</n-button>
            </div>
        </div>
        ...
        <div class="slot-group" :style="{ opacity: 计算机._当前机箱.装备的主板 ? 1 : 0.4 }">
            <n-text depth="3" class="slot-title">CPU插槽 - {{ 计算机._当前机箱.装备的CPU ? 1 : 0 }} / {{ 计算机.槽位限制().CPU }}</n-text>
            <div v-if="计算机._当前机箱.装备的CPU" class="hardware-item">
                <wu_pin_chao_lian_jie :id="计算机._当前机箱.装备的CPU" />
                <n-button size="tiny" type="error" ghost @click="计算机.卸载CPU(殖民地系统.当前视角ID)">拔出</n-button>
            </div>
            <div v-else-if="计算机.槽位限制().CPU > 0" class="hardware-empty">
                <n-select :value="null" :options="可用CPU选项" placeholder="空槽位 - 点击插上CPU..." size="small"
                    @update:value="(val) => 计算机.安装CPU(val,殖民地系统.当前视角ID)" />
            </div>
        </div>
        ...
        <div class="slot-group" :style="{ opacity: 计算机._当前机箱.装备的主板 ? 1 : 0.4 }">
            <n-text depth="3" class="slot-title">内存插槽 - {{ 计算机._当前机箱.装备的内存.length }} / {{ 计算机.槽位限制().内存 }}</n-text>
            <div v-for="(id, index) in 计算机._当前机箱.装备的内存" :key="'mem-' + index" class="hardware-item">
                ...
                <n-button size="tiny" type="error" ghost @click="计算机.卸载内存(index,殖民地系统.当前视角ID)">拔出</n-button>
            </div>
            <div v-for="i in 剩余内存槽位" :key="'empty-mem-' + i" class="hardware-empty">
                <n-select :value="null" :options="可用内存选项" placeholder="空槽位 - 点击插上内存..." size="small"
                    @update:value="(val) => 计算机.安装内存(val,殖民地系统.当前视角ID)" />
            </div>
        </div>
        ...
        <div class="slot-group" :style="{ opacity: 计算机._当前机箱.装备的主板 ? 1 : 0.4 }">
            <n-text depth="3" class="slot-title">硬盘插槽 - {{ 计算机._当前机箱.装备的硬盘.length }} / {{ 计算机.槽位限制().硬盘 }}</n-text>
            <div v-for="(id, index) in 计算机._当前机箱.装备的硬盘" :key="'hdd-' + index" class="hardware-item">
                <wu_pin_chao_lian_jie :id="id" />
                <n-tag size="small" type="warning" :bordered="false">{{ 格式化字节(获取物品数据(id)?.提供容量) }} | {{ 获取物品数据(id)?.存储类别 || '物体' }}</n-tag>
                <n-button size="tiny" type="error" ghost @click="卸载硬盘(index,殖民地系统.当前视角ID)">拔出</n-button>
            </div>
            <div v-for="i in 剩余硬盘槽位" :key="'empty-hdd-' + i" class="hardware-empty">
                <n-select :value="null" :options="可用硬盘选项" placeholder="空槽位 - 点击插上硬盘..." size="small"
                    @update:value="(val) => 计算机.安装硬盘(val,殖民地系统.当前视角ID)" />
            </div>
        </div>
    </n-card>
</template>
...
const 执行安装主板 = () => {
    if (待装主板.value) {
        计算机.安装主板(待装主板.value,殖民地系统.当前视角ID);
        待装主板.value = null;
    }
};

const 卸载主板 = () => {
    if (计算机._当前机箱.装备的内存.length > 0 || 计算机._当前机箱.装备的硬盘.length > 0 || 计算机._当前机箱.装备的CPU) {
        message.warning("必须先拔出所有内存、硬盘和CPU，才能拆除主板！");
        return;
    }
    计算机.卸载主板(殖民地系统.当前视角ID);
};

const 卸载硬盘 = (index) => {
    const 成功 = 计算机.卸载硬盘(index,殖民地系统.当前视角ID);
    if (!成功) {
...
.slot-title { font-size: 13px; margin-bottom: 8px; display: block; }
.hardware-item { display: flex; justify-content: space-between; align-items: center; padding: 10px 12px; background-color: rgba(24, 160, 88, 0.15); border: 1px solid v-bind('themeVars.successColor'); border-radius: 6px; margin-bottom: 8px; transition: all 0.2s; }
.hardware-empty { display: flex; justify-content: space-between; align-items: center; padding: 8px 12px; background-color: v-bind('themeVars.actionColor'); border: 1px dashed v-bind('themeVars.borderColor'); border-radius: 6px; margin-bottom: 8px; gap: 12px; }
</style>